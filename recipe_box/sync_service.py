...
def valid_peer_url(value: str) -> bool:
    ...
    return (
        parsed.scheme in {"http", "https"} and bool(parsed.netloc) and not parsed.username and not parsed.password and not parsed.fragment
    )
...
def preview_peer_changes(peer_id: str) -> dict:
    ...
    with db_connect() as conn:
        for remote in manifest["recipes"]:
            ...
            baseline = conn.execute(
                "SELECT checksum FROM sync_baselines WHERE peer_id = ? AND recipe_id = ?", (peer_id, remote["id"])
            ).fetchone()
    ...
    return {
        "peer_id": peer_id,
        "peer_name": peer["name"],
        "source_installation_id": manifest.get("installation_id", "unknown"),
        "items": items,
        "counts": counts,
    }
...
def run_peer_sync(peer_id: str) -> dict:
    ...
    peer = get_sync_peer(peer_id)
    started = now_iso()
    history_id = f"sync-{uuid.uuid4().hex}"
    imported = 0
    conflicts = 0
    try:
        with db_connect() as conn:
            ...
            if not owner:
                raise ValueError("Create a local account before importing recipes.")
            for item in preview["items"]:
                remote = item["recipe"]
                status = item["status"]
                if status == "new":
                    conn.execute(
                        "INSERT INTO recipes (id, owner_id, title, summary, prep_time, servings, ingredients_json, steps_json, category_key, created_at, updated_at, sync_source_installation_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                        (
                            remote["id"],
                            owner["id"],
                            remote["title"],
                            ...
                            remote["prep_time"],
                            remote["servings"],
                            json.dumps(remote["ingredients"]),
                            json.dumps(remote["steps"]),
                            remote.get("category", ""),
                            remote["created_at"],
                            remote["updated_at"],
                            preview["source_installation_id"],
                        ),
                    )
                    ...
                    existing_conflict = conn.execute(
                        "SELECT id FROM sync_conflicts WHERE peer_id = ? AND recipe_id = ? AND status = 'open'", (peer_id, remote["id"])
                    ).fetchone()
                    ...
                    conflicts += 1
                conn.execute(
                    "INSERT INTO sync_baselines (peer_id, recipe_id, checksum, remote_updated_at, updated_at) VALUES (?, ?, ?, ?, ?) ON CONFLICT(peer_id, recipe_id) DO UPDATE SET checksum=excluded.checksum, remote_updated_at=excluded.remote_updated_at, updated_at=excluded.updated_at",
                    (peer_id, remote["id"], recipe_checksum(remote), remote["updated_at"], now_iso()),
                )
            summary = dict(preview["counts"])
            summary["imported"] = imported
            conn.execute("UPDATE sync_peers SET last_sync_at = ? WHERE id = ?", (now_iso(), peer_id))
            conn.execute(
                "INSERT INTO sync_history (id, peer_id, started_at, finished_at, status, summary_json) VALUES (?, ?, ?, ?, ?, ?)",
                (history_id, peer_id, started, now_iso(), "conflict" if conflicts else "success", json.dumps(summary, sort_keys=True)),
            )
        return {"status": "conflict" if conflicts else "success", "summary": summary}
    except Exception:
        with db_connect() as conn:
            conn.execute(
                "INSERT INTO sync_history (id, peer_id, started_at, finished_at, status, summary_json) VALUES (?, ?, ?, ?, 'failure', ?)",
                (history_id, peer_id, started, now_iso(), json.dumps({"error": "sync failed"})),
            )
        raise


def resolve_sync_conflict_action(conflict_id: str, resolution: str) -> dict:
    if resolution not in {"keep_local", "use_remote", "keep_both"}:
        raise ValueError("resolution must be keep_local, use_remote, or keep_both")
    with db_connect() as conn:
        ...
        if resolution == "use_remote":
            conn.execute(
                "UPDATE recipes SET title=?, summary=?, prep_time=?, servings=?, ingredients_json=?, steps_json=?, category_key=?, updated_at=? WHERE id=?",
                (
                    remote["title"],
                    remote["summary"],
                    remote["prep_time"],
            ...
            conn.execute(
                "INSERT INTO recipes (id, owner_id, title, summary, prep_time, servings, ingredients_json, steps_json, category_key, created_at, updated_at, sync_source_installation_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'remote')",
                (
                    copy["id"],
                    owner,
                    copy["title"],
                    ...
                    copy["prep_time"],
                    copy["servings"],
                    json.dumps(copy["ingredients"]),
                    json.dumps(copy["steps"]),
                    copy.get("category", ""),
                    copy["created_at"],
                    copy["updated_at"],
                ),
