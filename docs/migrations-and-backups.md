...
The backup command uses SQLite's online backup API and verifies `PRAGMA integrity_check` before and after copying. Restore reports schema version and counts for users, recipes, inventory, ratings, comments, tags, and recipe/tag relationships. It refuses to overwrite an existing target unless `--force` is explicit.
